...
export function deleteNextTweet(
  ...
  total = 0,
  retries = 0,
  continueAfterRest = false,
) {
  // Selectors. This may change in the near future.
  const menuButtonSelector =
    '.css-175oi2r.r-kemksi.r-1kqtdi0.r-1ua6aaf.r-th6na.r-1phboty.r-16y2uox.r-184en5c.r-1abdc3e.r-1lg4w6u.r-f8sm7e.r-13qz1uu.r-1ye8kvj [d="M3 12c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm9 2c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm7 0c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2z"]';
  ...
  console.debug(
    "Continuar luego de descansar?",
    continueAfterRest,
    "Hay tweets eliminados?",
    total > 0,
    "Se eliminaron bastantes tweets",
    total > 0 && total % restAfterDeletedTweetsCount === 0,
  );

  if (
    !continueAfterRest &&
    total > 0 &&
    total % restAfterDeletedTweetsCount === 0
  ) {
    const restingSeconds = randomInteger(10, 30);
    console.warn(
      `Descansando ${restingSeconds} segundos antes de continuar...`,
    );
    setTimeout(() => {
  ...
  if (!menuButton) {
    console.info(
      "No encontré el botón del menú. Salteando al siguiente elemento.",
    );
    // Scroll all the way down for avoiding get into a loop of finding just retweets
  ...
  setTimeout(
    () => {
      ...
      const deleteOption = menuOptions.find((div) =>
        div.innerText.includes("Eliminar"),
      );

      // Maybe it's a retweet
      if (!deleteOption) {
        console.debug(
          'No encontré el botón de "Eliminar". Verificando si es un retweet.',
        );

        const retweetIcons = [
          ...menuButton
            .closest("article")
            .querySelectorAll(retweetIconSelector),
        ];
        ...
        if (!isRetweet) {
          ...
          console.error(
            'No encontré los botones de "Eliminar" ni de "Deshacer repost". Omitiendo el elemento.',
          );
          deleteNextTweet(index + 1, total, retries);
          return;
        }
        ...
        menuOptions[0]
          .closest("div[role=menu]")
          .previousSibling.previousSibling.click();
        ...
        setTimeout(
          () => {
            ...
            setTimeout(
              () => {
                const undoRetweetButton = [
                  ...document.querySelectorAll("div[role=menuitem] span"),
                ].find((div) => div.innerText.includes("repost"));

                if (!undoRetweetButton) {
                  console.error(
                    'Parecía un retweet pero no encontré el "Deshacer repost". Omitiendo el elemento.',
                  );
                  deleteNextTweet(index + 1, total, retries);
                  return;
                }
      ...
      setTimeout(
        () => {
          const deleteButton = [...document.querySelectorAll("button")].find(
            (div) => div.innerText.includes("Eliminar"),
          );
